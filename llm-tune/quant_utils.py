...
def get_fp_llama(path_to_act_scales, fp_features_num):
    ...
    for k, v in fp_indices_in_lin_layers.items():
        ...
        if layer_number not in outlier_ids.keys():
            outlier_ids[layer_number] = {rest_of_string: v.tolist()}
        else:
            outlier_ids[layer_number][rest_of_string] = v.tolist()
...
def prepare_llama_quant(path_to_act_scales, fp_features_num, **kwargs):
