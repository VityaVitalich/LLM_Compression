...
def glm_sequential(model, dataloader, act_scales, dev, args):
    ...
    layers = model.glm.transformer.layers
    save_dict = {}

    #model.model.embed_tokens = model.model.embed_tokens.to(dev)
    #model.model.norm = model.model.norm.to(dev)
    model.glm.word_embeddings.to(dev)
    model.glm.transformer.position_embeddings.to(dev)
    model.glm.transformer.block_position_embeddings.to(dev)
    ...
    class Catcher(torch.nn.Module):
        def __init__(self, module):
            super().__init__()
            self.module = module
        def forward(self, inp, mask, *args, **kwargs):
            inps[cache['i']] = inp
            cache['i'] += 1
    ...
    for i in range(len(layers)):
        ...
        for names in sequential:
            ...
            for name in subset:
                ...
                layer_scales = None
                if outlier_num > 0:
                    layer_scales = act_scales['glm.transformer.layers.{}.{}'.format(i, name)]
            ...
            for j in range(args.nsamples):
                outs[j] = layer(inps[j].unsqueeze(0), attention_mask)[0]
            for h in handles:
                h.remove()
